...
\begin{frame}[fragile]{Diagram}
	...
	\begin{tikzpicture}[every path/.style={<-, ultra thick}]
		...
		\uncover<2->{
		\node[nd = lightyellow] (preH) at (2.5,2.25) {$(\ch_0, \langle \cdot,\cdot \rangle_{\ch_0})$ \\ $\Bigl \langle \sum_i a_i K(\cdot, x_i), \sum_j b_j K(\cdot,x_j) \Big \rangle_{\ch_0} := 
        \va^T \mK \vb, \ \mK = \Bigl( K(x_i,x_j) \Bigr)_{i,j=1}^n
        ...
        {$(\ch_0, \langle \cdot,\cdot \rangle_{L^2})$ \\ $\Bigl \langle \sum_i a_i K(\cdot, x_i), \sum_j b_j K(\cdot,x_j) \Big \rangle_{L^2} := \sum_{i,j} a_i b_j \int_\Omega K(t,x_i) K(t,x_j) \, \varrho(t) \,\dif t$} edge (Hnaught);
        
        \uncover<7->{
		\node[above = .5cm of preLtwo.north, nd = lightgreen] (H)
		{$(\cf, \langle \cdot,\cdot \rangle_{L^2}) :=$  the completion of $(\ch_0, \langle \cdot,\cdot \rangle_{L^2})$ }
		edge (preLtwo);

        \uncover<8->{
        \node[nd = lightgreen] (preG) at (12.5,2.25)
        {$(\ch_0, \langle \cdot,\cdot \rangle_{\cg})$ \\ $\Bigl \langle \sum_i a_i K(\cdot, x_i), \sum_j b_j K(\cdot,x_j) \Big \rangle_{\cg} :=  \va^T \mK^{-1} \vb$} edge (Hnaught);
