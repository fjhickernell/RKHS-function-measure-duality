...
\begin{frame}[fragile]{Diagram}
	...
	\begin{tikzpicture}[every path/.style={<-, ultra thick}]
		...
		\uncover<2->{
		\node[nd = lightyellow] (preH) at (2.5,1.75)
        ...
        \uncover<5->{
        \node[above = .5cm of H.north, nd = lightyellow, text width = {}] (Riesz)
		{Riesz: $\forall L \in \ch', \ \exists g \in \ch \ni$ \\
		$L(f) = \langle g,f \rangle_{\ch} \ \forall f \in \ch$}
		edge (H) edge (linear);
...
%        	$\Bigl \langle \sum_i a_i K(\cdot, x_i), \sum_j b_j K(\cdot,x_j) \Big \rangle_{L^2} := \sum_{i,j} a_i b_j \int_\Omega K(t,x_i) K(t,x_j) \, \varrho(t) \,\dif t$}
        ...
        \uncover<8->{
        \node[nd = lightgreen] (preG) at (12.5,2)
        {$(\ch_0, \langle \cdot,\cdot \rangle_{\cg})$ \\ $\Bigl \langle \sum_i a_i K(\cdot, x_i), \sum_j b_j K(\cdot,x_j) \Big \rangle_{\cg} :=  \va^T \mK^{-1} \vb$};
