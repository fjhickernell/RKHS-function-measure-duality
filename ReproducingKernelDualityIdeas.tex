...
\begin{frame}[fragile]{Diagram}
	...
	\begin{tikzpicture}[every path/.style={<-, ultra thick}]
		...
		\uncover<2->{
		\node[nd = lightyellow] (preH) at (2.5,2.25) {$(\ch_0, \langle \cdot,\cdot \rangle_{\ch_0})$ \\ $\Bigl \langle \sum_i a_i K(\cdot, x_i), \sum_j b_j K(\cdot,x_j) \Big \rangle_{\ch_0} := \sum_{i,j} a_i K(x_i,x_j) b_j$};
