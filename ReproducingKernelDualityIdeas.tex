...
\newcommand{\preLtwonode}[2][]{\path #2 node[nd = lightgreen] (preLtwo)
	{$\langle f, g \rangle_{L^2} := \int_\Omega f(x) g(x) \, \varrho(x) \,\dif x$}}
...
\newcommand{\preGnode}[2][]{\path #2 node[nd = lightred] (preG)
	{$\langle f, g \rangle_{\cg} := \int_{\Omega^2} f(t) K(t,x) g(x) \, \varrho(t) \varrho(x) \,\dif t \dif x$}}
...
\begin{frame}{\tikz[baseline = 0ex]{\Hnaughtnode[font = {}, text width = {}]{}}}
	$K: \Omega \times \Omega \to \reals$ is called a \alert{kernel}; assume
	\begin{itemize}
		\item
	\end{itemize}
\end{frame}
