...
\begin{frame}[fragile]{Diagram}
	...
	\begin{tikzpicture}[every path/.style={<-, ultra thick}]
		...
		{$(\cg, \langle \cdot,\cdot \rangle_{L^2}) :=$  the completion of $(\ch_0, \langle 	\cdot,\cdot \rangle_{\cg})$};
		\draw[<-] (G) -- (preG);
        ...
        \uncover<11->{
		\node[nd = lightred] (HLtwo) at (\cpos,\upos)
		{$\ch = \{x \mapsto \langle K(\cdot,x), g \rangle_{L^2} : g \in \cg \}$}
		edge (H) edge (G);
