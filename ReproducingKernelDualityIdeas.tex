...
\begin{frame}[label = RKRd]{Reproducing Kernels for Functions on $\{1, \ldots, d\}$, aka Vectors}
	...
	\[
	\ip[\cf]{f}{h} : = \vf^T \mW \vh, \quad \forall f, h \in \cf, \qquad \text{where } \vf = \bigl(f(t) \bigr)_{t=1}^d
	\]
...
\begin{frame}[allowframebreaks]{Measures for Functions on $\{1, \ldots, d\}$}
	
	\vspace{-3ex}
	Let $\cm:=$ all signed measures on $\Omega := \{1, \ldots, d\} \text{ ``$=$'' } \reals^d$

 The Dirac measure, $\delta_x\in \cm$ corresponds to $\ve_x$, i.e., 
 \[
f(x) = \int_\Omega f(t) \, \delta_x(\dif t) = \ve_x^T \vf
 \]
 Any $\lambda = \lambda_1 \delta_1 + \cdots + \lambda_2 \delta_d \in \cm$ corresponds to $\vlambda = (\lambda_1, \ldots, \lambda_d) \in \reals^d$ such that 
\[
\int_\Omega f(t) \, \lambda(\dif t) = \vlambda^T \vf
\]
$\cm$ has an inner product defined as
\[
...
\[
f_\lambda(x) = \int_\Omega K(\vt,\vx) \, \lambda(\dif t)  \qquad \text{or} \qquad \vf_\lambda = \mK \vlambda
\]
For functions defined by this map
\[
\ip[\cf]{f_\lambda}{g_\mu} = \vf_\lambda^T \mW \vg_\mu \underbrace{=}_{\mK^T = \mK} \vlambda^T \mK \mW \mK \vmu \underbrace{=}_{\mK = \mW^{-1}} \vlambda^T \mK \vmu = \ip[\cm]{\lambda}{\mu}
\]

\end{frame}


\begin{frame}{Bounds on Linear Functionals}
