...
%Slide with ideas on reprocing kernel rigged Hilbert spaces
...
\colorlet{lightorange}{orange!20}
...
\tikzset{
	help lines /.append style ={dashed},
	>={Stealth[scale=0.7]},
	nd/.style ={draw = none,
		text width = 4.5cm,
		font = \small,
...
\newcommand{\upos}{4.7}
\newcommand{\uupos}{6}
\newcommand{\Hnaughtnode}[2][]{\path #2 node[nd = lightblue, #1] (Hnaught)
	{$\ch_0 = \spann\{ K(\cdot, x) : x \in \Omega\}$}}
\newcommand{\preHnode}[2][]{\path #2 node[nd = lightyellow, #1] (preH)
	{$\langle K(\cdot, t), K(\cdot,x) \rangle_{\ch_0} := K(t,x)$}}
\newcommand{\Hnode}[2][]{\path #2 node[nd = lightyellow, #1] (H)
	{$(\ch, \langle \cdot,\cdot \rangle_{\ch}) :=$  completion of $(\ch_0, \langle \cdot,\cdot \rangle_{\ch_0})$ }}
\newcommand{\linearnode}[2][]{\path #2 node[nd = lightorange, #1] (linear)
	{$\ch' := \{\text{bounded, linear}$ \\ $\text{functionals on } (\ch, \langle \cdot,\cdot \rangle_{\ch})\}$}}
\newcommand{\Riesznode}[2][]{\path #2 node[nd = lightyellow] (Riesz)
	{Riesz and reproducing: \\ $\forall L \in \ch'$, let $\eta_L(x) : = L(K(\cdot,x))$\\
	Then $L(f) = \langle \eta_L,f \rangle_{\ch} \ \forall f \in \ch$}}
\newcommand{\preLtwonode}[2][]{\path #2 node[nd = lightgreen] (preLtwo)
	{$\langle f, g \rangle_{L^2} := \int_\Omega f(x) g(x) \, \nu(\dif x) $}}
\newcommand{\Fnode}[2][]{\path #2 node[nd = lightgreen] (F)
	{$(\cf, \langle \cdot,\cdot \rangle_{L^2}) :=$  completion of $(\ch_0, \langle \cdot,\cdot \rangle_{L^2})$ }}
\newcommand{\preGnode}[2][]{\path #2 node[nd = lightred] (preG)
	{$\langle f, g \rangle_{\cg} := \int_{\Omega^2} f(t) K(t,x) g(x) \, \nu(\dif t) \nu(\dif x)$}}
\newcommand{\Gnode}[2][]{\path #2 node[nd = lightred] (G)
	{$(\cg, \langle \cdot,\cdot \rangle_{\cg}) :=$  completion of $(\ch_0, \langle 	\cdot,\cdot \rangle_{\cg})$}}
\newcommand{\GasHpnode}[2][]{\path #2node[nd = lightred] (GasHp)
	{$\forall L \in \ch', \exists g_L \in \cg \ \ni$ \\
	$L(f) = \langle g_L,f \rangle_{L^2} \ \forall f \in \ch$}}
\newcommand{\HLtwonode}[2][]{\path #2 node[nd = lightgreen] (HLtwo) at (\cpos,\upos)
	{$\ch = \{ f : f(x) = \langle K(\cdot,x), g \rangle_{L^2},$ \\ $g \in \cg \}$}}
...
\begin{frame}[fragile]{Diagram of Reproducing Kernel Hilbert Spaces and Their Relatives}

	\vspace{-3ex}

	\begin{tikzpicture}[every path/.style={<-, ultra thick}]
		%\draw[help lines] grid (15,7);
		\Hnaughtnode{(\cpos,0.25)};

		\uncover<2->{\preHnode{(\lpos,\dpos)};
        	\draw[<-] (preH) -- (Hnaught.west);}

        \uncover<3->{\Hnode{(\lpos,\mpos)} edge (preH);}

        \uncover<4->{\linearnode[text width = {}]{(\cpos,\uupos)};}

        \uncover<5->{\Riesznode{(\lpos,\upos)} edge (H) edge (linear);}

        \uncover<6->{\preLtwonode{(\cpos,\dpos)} edge (Hnaught);}

        \uncover<7->{\Fnode{(\cpos,\mpos)} edge (preLtwo);}

        \uncover<8->{\preGnode{(\rpos,\dpos)};
        \draw[<-] (preG) -- (Hnaught.east);}

        \uncover<9->{\Gnode{(\rpos,\mpos)};
		\draw[<-] (G) -- (preG);}

        \uncover<10->{\GasHpnode{(\rpos,\upos)} edge (G);
		\draw[<-] (GasHp) -- (linear);}

        \uncover<11->{\HLtwonode{(\cpos,\upos)} edge[->] (H) edge (G);}
...
\begin{frame}{\tikz[baseline = (Hnaught.base)]{\Hnaughtnode[font = {}, text width = {}]{}}}
\begin{equation*}
	\ch_0 = \left \{c_1 K(\cdot,x_1) + \cdots + c_n K(\cdot,x_n) : \vc \in \reals^n, \ \vx \in \Omega^n, \ n \in \naturals \right\}
\end{equation*}

\vspace{3ex}
\uncover<2->{Examples where $\{K(\cdot,x) : x \in \Omega\}$ may be finite or infinite
\begin{gather*}
	\Omega = \{1, \ldots, m\}, \qquad K(t,x) = \begin{cases} 1, & t=x \\ 0, & t \ne x \end{cases}, \qquad \ch_0 = \reals^m \\
	\Omega = [0,1], \qquad K(t,x) = 2 - \abs{t-x} \\
	\Omega = \reals, \qquad K(t,x) = \exp(-(t-x)^2)
\end{gather*}}
\end{frame}

\begin{frame}{\tikz[baseline = (preH.base)]{\preHnode[font = {}, text width = {}]{}}}
	$K: \Omega \times \Omega \to \reals$ is called a (reproducing) \alert{kernel}; assume
	\begin{description}[\labelwidth = 20ex]
		\item[Symmetry] $K(t,x) = K(t,x)$ for all $t,x \in \Omega$
		\item[Positive definiteness] $\sum_{i,j=1}^n c_ic_jK(x_i,x_j) = \vc^T \mK \vc > 0$ for all $\vzero \ne \vc \in \reals^n$, distinct $x_1, \ldots, x_n \in \Omega$, $n \in \naturals$
	\end{description}

	Let $\langle \cdot, \cdot \rangle_{\ch_0} : \ch_0 \times \ch_0 \to \reals$ be a symmetric, bilinear form defined as above.  Then for all $ f = \sum_i a_i K(\cdot, x_i), \ g =  \sum_j b_j K(\cdot, x_j) \in \ch_0$, with distinct $x_1, x_2, \ldots$
	\begin{align*}
		\langle f,g \rangle & =\Bigl \langle \sum_i  a_i K(\cdot, x_i), \sum_j b_j K(\cdot, x_j) \Bigr \rangle =  \sum_{ij} a_i b_j \langle K(\cdot, x_i), K(\cdot, x_j) \rangle = \sum_{ij} a_i b_j  K( x_i, x_j) \\
		& = \sum_{ij} b_i a_j K( x_i, x_j) = \langle g, f \rangle \\
		\langle f, f\rangle &= \sum_{ij} a_i a_j K( x_i, x_j) > 0 \qquad \forall f \ne 0
	\end{align*}

\end{frame}

\begin{frame}{\tikz[baseline = (preH.base)]{\preHnode[font = {}, text width = {}]{}} cont'd}
\begin{itemize}
	\item $(\ch_0,\langle \cdot, \cdot \rangle_{\ch_0})$ is an \alert{inner product space}
	\item $K(\cdot,x)$ is the \alert{representer} of the evaluation functional because for all $f = \sum_{i} a_i K(\cdot,x_i) \in \ch_0$,
	\[
	\langle K(\cdot,x),f\rangle_{\ch_0}
	= \Bigl \langle K(\cdot,x), \sum_{i} a_i K(\cdot,x_i) \Bigr \rangle_{\ch_0}
	= \sum_{i} a_i \langle K(\cdot,x),  K(\cdot,x_i) \rangle_{\ch_0}
	= \sum_{i} a_i K(x,x_i) = f(x)
	\]
\end{itemize}

\end{frame}

\begin{frame}{\tikz[baseline = (H.base)]{\Hnode[font = {}, text width = {}]{}}}

This means adding the limits of \alert{Cauchy sequences}, e.g., $\ch_0 = \mathbb{Q}$ and $\ip[\ch_0]{f}{g} = fg$ leads to $\ch = \reals$

Examples
\begin{gather*}
	\Omega = \{1, \ldots, m\}, \qquad K(t,x) = \begin{cases} 1, & t=x \\ 0, & t \ne x \end{cases}, \qquad \ch = \ch_0 \cong \reals^m, \qquad \ip[\ch]{f}{g} = \sum_{x = 1}^m f(x) g(x) \\
	\Omega = [0,1], \qquad K(t,x) = 2 - \abs{t-x}, \qquad  \ip[\ch]{f}{g} = \\
	\Omega = \reals, \qquad K(t,x) = \exp(-(t-x)^2), \qquad  \ip[\ch]{f}{g} =
\end{gather*}

\vspace{-3ex}
\begin{itemize}
	\item $K(\cdot,x)$ is the \alert{representer} of the evaluation functional, i.e., $f(x) = \langle K(\cdot,x),f\rangle_{\ch}$ for all $f\in \ch$
	\item If $(\tch,\ip[\tch]{\cdot}{\cdot})$ is a Hilbert space with dense subspace $(\ch_0,\ip[\ch_0]{\cdot}{\cdot})$, then $(\tch,\ip[\tch]{\cdot}{\cdot}) = (\ch,\ip[\ch]{\cdot}{\cdot})$
	\item $\ip[\ch]{f}{g}$ has to be \alert{guessed and verified} for $f,g \in \ch \setminus \ch_0$
\end{itemize}
\end{frame}
...
Then
% \begin{itemize}
%     \item $\ip{\cdot}{\cdot}$ is bilinear for $\cm_1 \times \cf_1$.
%     \item If $\norm[\cf_1]{\cdot} = \sqrt{\ip[\cf_1 \times \cf_1]{\cdot}{\cdot}}$, and $\cf$ is the completion of $\cf_1$ under $\norm[\cf_1]{\cdot}$
% \end{itemize}
\end{theorem}
